...
contract VanityAddress is Script, StdAssertions {
                ...
                uint256( // convert to uint to truncate upper digits.
                    keccak256( // compute the CREATE2 hash using 4 inputs.
                        abi.encodePacked( // pack all inputs to the hash together.
                                ...
                                Constants.IMMUTABLE_CREATE2_FACTORY_ADDRESS, // this contract will be the caller.
    ...
    function minePrefix(
        ...
        uint256 initInt,
        uint256 iterations
    ) internal pure returns (address computedAddress, uint256 i) {
        // Deploy diamond
        i = initInt;
        bool found = false;
        while (!found && i - initInt < iterations) {
            computedAddress = _findDeploymentAddress(
                bytes32(
                    abi.encodePacked(deployer, abi.encodePacked(uint96(i)))
                ),
                initCode
            );
    ...
    function mine(
        ...
        while (!found && i - initInt < iterations) {
            ...
            if (
                uint24(uint160(bytes20(computedAddress))) == postfix &&
                uint24(uint160(bytes20(computedAddress)) >> 136) == prefix
            ) {
                found = true;
                break;
            }
